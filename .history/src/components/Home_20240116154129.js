...
const Home = () => {
  ...
  // this function calculates the result of the mathematical operations defined by the rows in the calculator, considering the signs (+ or -) and the enabled/disabled status of each row. The result is the sum or difference of the values in the enabled rows.
  const resultCal = () => {
    return rows.reduce(
      (oldResult, row) =>
        row.enabled
