...
const Home = () => {
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <div className="w-full flex gap-5 bg-blue-500 p-2 rounded-md text-white uppercase justify-center items-center">
        Result : <span className={`${resultCal() < 0 ? "bg-red-600" : ""}`}>{resultCal()}</span>
      </div>
    </div>
  );
};
